...
/* The control room — this is the "we have to be mods" requirement, and it turned out
   ...
   `writeApi` is empty on the deployed app and points at the local FastAPI service when you
   run `make api`. Everything that spends money or rewrites a persona card is hidden without
   it, because those endpoints deliberately do not exist on Workers — DEPLOY.md §3. The two
   reads are same-origin route handlers and work in both places. */
...
export default function AdminConsole({ writeApi }: { writeApi: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      [c, s] = await Promise.all([
        fetch(`/api/admin/characters`, { headers: headers() }),
  ...
  async function patch(id: string, body: Record<string, unknown>) {
    await fetch(`${writeApi}/api/admin/characters/${id}`, {
      method: "PATCH",
      headers: headers(),
  ...
  async function runTick() {
    setNote("Ticking…");
    const res = await fetch(`${writeApi}/api/admin/tick`, { method: "POST", headers: headers() });
  ...
  async function draftCharacter(e: React.FormEvent) {
    e.preventDefault();
    setNote("Drafting…");
    const res = await fetch(`${writeApi}/api/admin/characters/draft`, {
      method: "POST",
      headers: headers(),
    ...
    <div className="admin">
      ...
      {!writeApi && (
        <p className="board-note">
          Read-only. The tick trigger, persona edits and drafting run against the local API —
          start it with <code>make api</code> and reload.
        </p>
      )}

      <h2>Cast{writeApi ? " — posting rate is the throttle" : ""}</h2>
      {characters.map((c) => (
        <div className="crow" key={c.id}>
          ...
          {!writeApi && <div className="crow-input">{c.engagement_profile?.opens_per_day ?? 1}</div>}
          {!writeApi && <div className="crow-input">{c.status}</div>}
