...
export const SQL: Record<QueryName, { text: string; params: string[] }> = {
  ...
  insert_human_poke: { text: "WITH new_post AS (\n    INSERT INTO posts (world_id, author_human, body, parent_id, root_id)\n    VALUES ($1, $2, $3, $4, $5)\n    RETURNING id\n), bumped AS (\n    UPDATE posts SET reply_count = reply_count + 1 WHERE id = $4\n), notified AS (\n    INSERT INTO notifications (character_id, post_id, kind)\n    SELECT CAST($6 AS uuid), new_post.id, 'human'\n      FROM new_post\n     WHERE CAST($6 AS uuid) IS NOT NULL\n)\nSELECT id FROM new_post", params: ["world","author","body","parent","root","character"] },
